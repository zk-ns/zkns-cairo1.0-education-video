...
trait NGTToken {
    #[external]
    fn pool_mint(recipient: ContractAddress, amount: u256);
    #[external]
...
mod Stake {
    ...
    #[external]
    fn set_token_address(_ngttoken_address: ContractAddress){
    ...
    fn make_order(amount: u256, kinds: u8){
       assert(kinds ==1 | kinds ==2 | kinds ==3, 'kinds not allow');
       let caller: ContractAddress = get_caller_address();
    ...
    fn claim_order(_index: u64){
       ...
       assert(seedmoney > 0, 'have claimed');
       if(kinds == 1){
           if(get_block_timestamp() - starttime >= FIRSTDIFF){
              ...
              NGTTokenDispatcher { contract_address: ngttoken_address::read() }.pool_mint(caller, profit_amount_);
           }else{
